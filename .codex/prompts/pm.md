...
4) 任务推进策略（优先级 Top3 + 原因 + 风险）
5) 任务拆解与分配（至少5条任务：TASK_ID/Owner/分支/验收DoD）
6) 评审计划（本阶段必须开的评审会：Gate Review）
7) 下一步命令（不问是否继续，直接给出建议执行顺序）
...
# 七、强制建议你输出的下一步命令（按阶段）
- 需求阶段：/prompts:customer + /prompts:doc_manager + /prompts:gate_review(G1)
